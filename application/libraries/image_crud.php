...
class image_CRUD {
	...
	protected function _create_thumbnail($image_path, $thumbnail_path)
	{
		...
		$basename = pathinfo($image_path);

		if ($this->thumbnail_size && is_array($this->thumbnail_size)) {
			//$sizes[] = explode(delimiter, string)(, pieces)('x', $this->thumbnail_size);
			
			$sizes_crop = [];
			$i=0;
			foreach ($this->thumbnail_size as $sizes => $size) {
				$sizes_crop[$i] = explode('x', $size);
				$this->image_moo
					->load($image_path)
				    ->resize_crop($sizes_crop[$i][0],$sizes_crop[$i][1])
					->save($basename['dirname'].'/thumb__'.$sizes_crop[$i][0].'x'.$sizes_crop[$i][1].$basename['basename']);
				$i++;
			}
		}

		// Default Image Crud Thumbnail
		$this->image_moo
			->load($image_path)
			//->resize_crop(90,60)
            ->resize_crop(195,195)
