...
class Subscriber extends Admin_Controller {
    ...
    public function __construct() {
            ...
            // Load Gallery model
            //$this->load->model('member/Gallery');
    ...
    public function index() {
	    ...
	    // Set our Grocery CRUD
            ...
            // Set tables
            $crud->set_table($this->Members->table)->order_by('join_date','desc');
          // Works exactly the same way as codeigniter's where ($this->db->where)
            $crud->where('type','Subscriber');
            // Set CRUD subject
            $crud->set_subject('Subscribers');
            // Set table relation
            ...
            $crud->display_as('about','Informasi');
            $crud->display_as('research_area','Product');
            $crud->display_as('phone_number','Phone');

            // Set column
            $crud->columns('fullname','email','phone_number','about','research_area','address','province','urbandistrict','suburban','join_date');

            // Set fields
            $crud->fields('fullname','email','phone_number','about','research_area','address','province','urbandistrict','suburban','type','join_date');
			...
			$crud->callback_add_field('type',array($this,'_callback_type'));
			...
			//$crud->unset_edit();
			//$crud->unset_delete();
            $this->load($crud, 'member');
        } catch (Exception $e) {
            show_error($e->getMessage() . ' --- ' . $e->getTraceAsString());
        }
    }

    public function _callback_type () {
        return '<input type="hidden" maxlength="14" value="Subscriber" name="type">';
    }
    ...
    public function _callback_total_image($value, $row) {
        $total = $this->user_model->total_image_submitted($row->member_id);
        return $total;
    }
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Subscribers Title
            $output->page_title = 'Subscribers Listings';
            // Set Main Template
