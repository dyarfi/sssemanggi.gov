...
class Settings Extends MY_Model {
	...
	public function install() {
		...
		if (!$this->db->table_exists($this->table)) 
                $insert_data	= TRUE;
               	...
               	$sql	= 'CREATE TABLE IF NOT EXISTS `'.$this->table.'` ('
					...
					. '`input_type` enum(\'input\',\'image\',\'textarea\',\'radio\',\'dropdown\',\'timezones\') DEFAULT NULL,'
	    ...
	    if ($insert_data) {
		$sql	= 'INSERT INTO `'.$this->table.'` (`id`, `parameter`, `group`, `alias`, `value`, `status`, `added`, `modified`) VALUES '
			    ...
			    .'(13, \'site_logo\',\'\', \'Site Logo\', \'logo_small.png\', \'1\', NULL, 1336118568), '	
	...
	public function getByParameter($param = null){
		if(!empty($param)){
			$data = array();
			$options = array('parameter' => $param,'status'=>1);
			$Q = $this->db->get_where($this->table,$options,1);
			if ($Q->num_rows() > 0){
				foreach ($Q->result_object() as $row)
				$data = $row;
	...
	public function getAllSetting($status=null){
		$data = array();
		$this->db->order_by('group','DESC')->order_by('added');
	...
	public function getAllByGroup($group=null){
		...
		$this->db->order_by('group')->order_by('added');
		$Q = $this->db->get($this->table);
			if ($Q->num_rows() > 0){
				$data = $Q->result_object();
			}
