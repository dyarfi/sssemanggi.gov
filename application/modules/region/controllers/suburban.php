...
class SubUrban extends Admin_Controller {
    
    public function __construct() {
            parent::__construct();

            // Load Participant model
            //$this->load->model('region/SubDistrict');
    ...
    public function index() {
	    ...
	    // Set our Grocery CRUD
            ...
            // Set tables
            $crud->set_table('tbl_sub_urbans')->order_by('name','ASC');
            // Set CRUD subject
            $crud->set_subject('Sub Urban');                            
            // Set table relation
            $crud->set_relation('urban_district_id','tbl_urban_districts','name',null,'name ASC');
            // Set column
            $crud->columns('name','urban_district_id');			
			// The fields that user will see on add and edit form
			$crud->fields('name','urban_district_id','added','modified');
			// Set column display 
			$crud->display_as('urban_district_id','Urban District');
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            ...
            $output->page_title = 'Sub Urban Listings';
            // Set Main Template
