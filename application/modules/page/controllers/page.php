...
class Page extends Admin_Controller {
    ...
    public function index() {
	    ...
	    // Set our Grocery CRUD
            ...
            // Order by Listing
            $crud->order_by('added','DESC');
            // Set CRUD subject
            $crud->set_subject(lang('Pages'));                            
            // Set table relation
            $crud->set_relation('menu_id','tbl_page_menus','subject',['status'=>'publish'],'id DESC');
            // Set columns
            $crud->columns('subject','menu_id','text','media','status','added','modified');			
			// The fields that user will see on add and edit form
			$crud->fields('subject','url','menu_id','text','media','user_id','status','added','modified');
            // Set column display 
			...
			if ($this->Languages->getActiveCount() > 1) {
				...
				$crud->columns('subject','menu_id','text','media','status','added','modified','translate');			
    ...
    public function _callback_translate ($value, $row) {
		...
		foreach($this->Languages->getAllLanguage(['status'=>1]) as $lang) {			
			...
			if($lang->default != 1) {
				$page_db = $this->db->where('lang_id',$lang->id)->where('field_id',$row->id)->where('table','tbl_pages')->get('tbl_translations');
    ...
    public function _callback_gallery ($value,$row) {
   	...
   	public function _callback_after_delete($primary_key) {
	    ...
	    return $this->db->delete('tbl_translations',['field_id' => $primary_key, 'table' => 'tbl_pages']);
	}
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Page Title 
            $output->page_title = lang('Pages').' Listings';
            // Set Main Template
