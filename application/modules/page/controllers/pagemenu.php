...
class PageMenu extends Admin_Controller {
    ...
    public function index() {
	    ...
	    // Set our Grocery CRUD
            ...
            // Set CRUD subject
            $crud->set_subject(lang('Page Menus'));
            // Set table relation
            $crud->set_relation('group_id','tbl_page_groups','subject',['status'=>'publish'],'priority ASC');
            // The fields that user will see on add and edit form
			$crud->fields('subject','group_id','url','ext_url','text','type','position','media','priority','user_id','status','added','modified');
            // Set column
            $crud->columns('subject','group_id','position','media','status','modified');
            // Unsets the fields at the add form.
			$crud->unset_add_fields('parent_id','sub_level','has_child','order','count','is_system','added','modified');
			// Unsets the fields at the edit form.
			$crud->unset_edit_fields('parent_id','sub_level','has_child','order','count','is_system','added','modified');
            // Set custom field display for position
            $crud->field_type('position','dropdown',array('top'=>'Top','bottom'=>'Bottom','top_bottom'=>'Top & Bottom'));
   	...
   	public function _callback_after_delete($primary_key) {
	    ...
	    return $this->db->delete('tbl_translations',['field_id' => $primary_key, 'table' => 'tbl_page_menus']);
	}
    ...
    public function _callback_priority_add ($value, $row) {
        ...
        $count = $this->PageMenus->find_count(['status !='=>'']) + 1;
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Page Title
            $output->page_title = lang('Page Menus').' Listings';
            // Set Main Template
