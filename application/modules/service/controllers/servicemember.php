...
class ServiceMember extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            $crud->where("approved = '1'")->where("type = 'Service'");
            // Order by Listing
            ...
            // Set allowed access
            if(!$this->is_allowed[$this->class.'/member/index/delete']) {
                $crud->unset_delete();
            }
            if(!$this->is_allowed[$this->class.'/member/index/edit']) {
                $crud->unset_edit();
            }
            if(!$this->is_allowed[$this->class.'/member/index/read']) {
                $crud->unset_read();
            }
            if(!$this->is_allowed[$this->class.'/member/index/add']) {
                $crud->unset_add();
            }
            $crud->unset_print();
            $crud->unset_export();
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Member').' Listings';
            // Set Main Template
