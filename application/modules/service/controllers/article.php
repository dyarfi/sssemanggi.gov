...
class Article extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            // Set CRUD subject
            $crud->set_subject(lang('Article'));
            // Set columns
            $crud->columns('subject','text','media','publish_date', 'type' ,'status','added','modified');
            // Set column type callback
            $crud->callback_column('type',array($this,'_callback_column_type'));
            // The fields that user will see on add and edit form
            $crud->fields('subject','url','text','media','type', 'publish_date','user_id','status','added','modified');
...
//            $crud->required_fields('subject','text','publish_date','status', 'type');
            ...
            // Set allowed access
            if(!$this->is_allowed[$this->class.'/article/index/delete']) {
                $crud->unset_delete();
            }
            if(!$this->is_allowed[$this->class.'/article/index/edit']) {
                $crud->unset_edit();
            }
            if(!$this->is_allowed[$this->class.'/article/index/read']) {
                $crud->unset_read();
            }
            if(!$this->is_allowed[$this->class.'/article/index/add']) {
                $crud->unset_add();
            }
            $crud->unset_print();
            $crud->unset_export();
    ...
    public function _callback_field_type($value = '', $primary_key = null) {
        ...
        return '<select name="type">
                <option value="svc-artc">Article</option>
                <option value="svc-evt">Event</option>
                <option value="svc-blog">Blog</option>
                </select>';
    }
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Article').' Listings';
            // Set Main Template
