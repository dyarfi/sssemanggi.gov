...
//            $crud->where("type = 'svc-artc' OR type = 'svc-evt' OR type = 'svc-blog'");
            // Order by Listing
            $crud->order_by('sts_publish_date','DESC');
            // Set tables
            $crud->set_table('tbl_service_tips');
            // Set CRUD subject
            $crud->set_subject(lang('Tips'));
            // Set columns
            $crud->columns('sts_subject','sts_text','sts_publish_date','sts_type','sts_status','sts_added','sts_modified');
            // The fields that user will see on add and edit form
            $crud->fields('sts_subject','sts_url','sts_text','sts_type','sts_media','sts_publish_date','sts_status','sts_added','sts_modified');

            $crud->display_as('sts_subject', 'Subject')
                ->display_as('sts_text', 'Text')
                ->display_as('sts_publish_date', 'Publish Date')
                ->display_as('sts_type', 'Type')
                ->display_as('sts_status', 'Status')
                ->display_as('sts_added', 'Added')
                ->display_as('sts_modified', 'Modified');
            // Changes the default field type
            $crud->field_type('sts_url', 'hidden');
            $crud->field_type('sts_added', 'hidden');
            $crud->field_type('sts_modified', 'hidden');
            // Set unique fields
...
//            }
            ...
            $crud->callback_add_field('sts_added',array($this,'_callback_time_added'));
            // This callback escapes the default auto field output of the field name at the edit form
            $crud->callback_edit_field('sts_modified',array($this,'_callback_time_modified'));
            // This callback escapes the default auto field output of the field name at the add/edit form.
            $crud->callback_field('sts_type',array($this,'_callback_field_type'));
            ...
            $crud->callback_column('sts_added',array($this,'_callback_time'));
            $crud->callback_column('sts_modified',array($this,'_callback_time'));
...
//            $crud->required_fields('subject','text','publish_date','status', 'type');
            $crud->set_rules('sts_subject','Subject','trim|required|min_length[3]|max_length[128]|xss_clean');
            $crud->set_rules('sts_text','Text','trim|min_length[3]|xss_clean');

            // Set upload field
            $crud->set_field_upload('sts_media','uploads/news', 'jpg|jpeg');

            // Changes the displaying label of the field.
            $crud->display_as('sts_media','Image');
...
//        return $this->db->delete('tbl_translations', ['field_id' => $primary_key, 'table' => 'tbl_news']);
    ...
    public function _callback_url($value, $primary_key) {
        ...
        $value['sts_url'] = url_title($value['sts_subject'],'-',true);
    ...
    public function _callback_time_added ($value, $row) {
        $time = time();
        return '<input type="hidden" maxlength="50" value="'.$time.'" name="sts_added">';
    }

    public function _callback_time_modified ($value, $row) {
        $time = time();
        return '<input type="hidden" maxlength="50" value="'.$time.'" name="sts_modified">';
    }
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Tips').' Listings';
            // Set Main Template
