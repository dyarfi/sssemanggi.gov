...
class Booking_approval extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            $crud->where("tbl_service_booking.approved = '0'");
            // Order by Listing
...
//            $crud->set_relation_n_n('tags', 'tbl_tags_to_news', 'tbl_tags', 'news_id', 'tag_id', 'subject', 'priority');
            ...
            $crud->columns('name','email','phone','vin_number','license_number','dealer_id', 'service_type', /*'service_package',*/ 'time', 'date');            

            $crud->display_as('dealer_id', 'Dealer')
                ->display_as('service_member_id', 'Member')
                ->display_as('service_vehicle_id', 'VIN')
                ->display_as('service_type', 'Tipe Servis')
...
//            $crud->callback_column('type',array($this,'_callback_column_type'));
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Booking Approval').' Listings';
            // Set Main Template
