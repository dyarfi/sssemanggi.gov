...
class ProductGroup extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            $crud->order_by('priority','ASC');
         	// Set tables
            $crud->set_table($this->ProductGroups->table);
            // Set CRUD subject
            $crud->set_subject(lang('Product Group'));
            // Set columns
            $crud->columns('subject','type','text','priority','status','added','modified');
			// The fields that user will see on add and edit form
			$crud->fields('subject','type','url','text','priority','user_id','status','added','modified');
    ...
    public function _callback_gallery ($value,$row) {
   	...
   	public function _callback_media ($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/products').'/'.$value.'" class="fancyframe iframe"><img height="40" src="'.base_url('uploads/products').'/'.$value.'"/></a>';
        } else {
            return '-';
        }
    }
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Page Title
            $output->page_title = lang('Product Group').' Listings';
            // Set Main Template
