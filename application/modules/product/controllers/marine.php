...
class Marine extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            $crud->where('tbl_products.type','marine');
			// Set tables
            $crud->set_table($this->Products->table);
            // Set relation
            $crud->set_relation('category_id','tbl_product_categories','subject',array('status' => 'publish'));
            // Set relation
            $crud->set_relation('group_id','tbl_product_groups','subject',array('type' => 'marine'),'subject ASC');
            // Order by Listing
            $crud->order_by('priority','DESC');
            // Set CRUD subject
            $crud->set_subject(lang('Marine'));
            // Set columns
            $crud->columns('subject','category_id','group_id','text','cover','media','media_plain','model','service','status','added','modified');
			// The fields that user will see on add and edit form
			$crud->fields('subject','url','category_id','group_id','text','cover','thumbnail','media','media_plain','specification','price','messages','brochure','pricelist','priority','status','type','is_service','is_pricelist','added','modified');
			...
			$crud->field_type('is_service','true_false',array('0' => 'No', '1' => 'Yes'));
			$crud->field_type('is_pricelist','true_false',array('0' => 'No', '1' => 'Yes'));
            ...
            // Set upload field
            $crud->set_field_upload('media_plain','uploads/automobile', $this->notes->media['ext']);
			...
			$crud->required_fields('subject','category_id','text','status');
            ...
            // Set upload field
            $crud->set_field_upload('cover','uploads/marine', $this->notes->cover['ext']);
            // Set upload field
            $crud->set_field_upload('media','uploads/marine', $this->notes->cover['ext']);
           // Set upload field
            $crud->set_field_upload('media_plain','uploads/marine', $this->notes->cover['ext']);
            // Set upload field
            $crud->set_field_upload('thumbnail','uploads/marine', $this->notes->cover['ext']);
    ...
    public function _callback_after_upload($uploader_response,$field_info, $files_to_upload) {
        ...
        $thumbnail[1]      = $field_info->upload_path.'/thumb__600x600'.$uploader_response[0]->name;
	...
	public function _callback_cover ($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/marine').'/'.$value.'" class="fancyframe iframe"><img height="30" src="'.base_url('uploads/marine').'/'.$value.'"/></a>';
        } else {
            return '-';
        }
    }
	...
	public function _callback_column_media($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/marine').'/'.$value.'" class="fancyframe iframe"><img height="40" src="'.base_url('uploads/marine').'/'.str_replace($value, 'thumb__1192x671'.$value, $value).'"/></a>';
        } else {
            return '-';
        }
    }
  	...
  	public function _callback_field_type($value = '', $primary_key = null) {
		...
		return '<input type="hidden" maxlength="50" value="marine" name="type">';
	}
				...
				'spesifikasi' => [
			    	'specifications' => [
			    		'attributes' => [
							...
							[
		    					'name'	=> 'full_throttle_operating_range',
			    				'label' => 'Full Throttle Operating Range',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							...
							[
		    					'name'	=> 'trim_method',
			    				'label' => 'Trim Method (Positions)',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							[
		    					'name'	=> 'fuel_oil_mixing_ratio',
			    				'label' => 'Fuel/Oil Mixing Ratio',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							...
							[
		    					'name'	=> 'drive_protection',
			    				'label' => 'Drive Protection',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				]
		    		...
		    		'attributes' => [
		    				...
		    				[
		    					'name'	=> 'start_in_gear_Protection',
			    				'label' => 'Start-in Gear Protection',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
		    				[
		    					'name'	=> 'shallow_water_drive',
			    				'label' => 'Shallow Water Drive',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
		    				[
		    					'name'	=> 'loop_charge_fuel_induction',
			    				'label' => 'Loop Charge Fuel Induction',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							[
		    					'name'	=> 'dual_water_intakes',
			    				'label' => 'Dual Water Intakes',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							[
		    					'name'	=> 's_s_water_pump_housing',
			    				'label' => 'S.S. Water Pump Housing',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							[
		    					'name'	=> 'anti_corrosion_system',
			    				'label' => 'Anti-Corrosion System',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				]
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Marine').' Listings';
            // Set Main Template
...
/* End of file marine.php */
/* Location: ./application/module/product/controllers/marine.php */
