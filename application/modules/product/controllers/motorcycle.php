...
class Motorcycle extends Admin_Controller {
    ...
    public function index() {
        try {
            ...
            $crud->where('tbl_products.type','motorcycle');
			// Set tables
            $crud->set_table($this->Products->table);
            // Set relation
            $crud->set_relation('category_id','tbl_product_categories','subject',array('status' => 'publish'));
            // Set relation
            //$crud->set_relation('group_id','tbl_product_groups','subject',array('type' => 'motorcycle'),'subject ASC');
            // Order by Listing
            $crud->order_by('priority','DESC');
            // Set CRUD subject
            $crud->set_subject(lang('Motorcycle'));
            // Set columns
            $crud->columns('subject','category_id','text','cover','media','media_plain','model','variant','accessory','service','status','added','modified');
			// The fields that user will see on add and edit form
			$crud->fields('subject','url','category_id','text','cover','thumbnail','media','media_plain','specification','feature','price','messages','brochure','pricelist','priority','status','type','is_service','is_pricelist','added','modified');
			...
			if ($this->Languages->getActiveCount() > 1) {
				...
				$crud->columns('subject','text'/*,'gallery'*/,'media','media_plain','status','added','modified','translate');
			...
			// -------------------- SERVICE ---------------------------- //
			// This callback escapes the default auto field output of the field name at the edit form
			$crud->callback_field('service',array($this,'_callback_service'));
            // -------------------- Brochure ---------------------------- //
            // This callback escapes the default auto field output of the field name at the edit form
            ...
            // Set upload field
            $crud->set_field_upload('cover','uploads/motorcycle', $this->notes->cover['ext']);
            // Set upload field
            $crud->set_field_upload('media','uploads/motorcycle', $this->notes->cover['ext']);
            // Set upload field
            $crud->set_field_upload('media_plain','uploads/motorcycle', $this->notes->cover['ext']);
            // Set upload field
            $crud->set_field_upload('thumbnail','uploads/motorcycle', $this->notes->cover['ext']);
            ...
            $crud->unset_export();
            // Load Grocery Crud
            $this->load($crud, 'motorcycle_crud');
        } catch (Exception $e) {
	...
	public function _callback_cover ($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/motorcycle').'/'.$value.'" class="fancyframe iframe"><img height="30" src="'.base_url('uploads/motorcycle').'/'.$value.'"/></a>';
        } else {
            return '-';
        }
    }
	...
	public function _callback_media ($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/motorcycle').'/'.$value.'" class="fancyframe iframe"><img height="20" src="'.base_url('uploads/motorcycle').'/'.$value.'"/></a>';
        } else {
            return '-';
        }
    }
	...
	public function _callback_column_media($value,$row) {
	    if ($value) {
            return '<a href="'.base_url('uploads/motorcycle').'/'.$value.'" class="fancyframe iframe"><img height="40" src="'.base_url('uploads/motorcycle').'/'.str_replace($value, 'thumb__1192x671'.$value, $value).'"/></a>';
        } else {
            return '-';
        }
    }
    ...
    public function _callback_column_model($value,$row) {
  	...
  	public function _callback_field_type($value = '', $primary_key = null) {
		...
		return '<input type="hidden" maxlength="50" value="motorcycle" name="type">';
	}
	    	...
	    	$jsons = [
				'spesifikasi' => [
		    		...
		    		'attributes' => [
		    				...
		    				[
		    					'name'	=> 'diameter_langkah',
			    				'label' => 'Diameter Langkah',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
		    				...
		    				[
		    					'name'	=> 'ratio_kompresi',
			    				'label' => 'Ratio Kompresi',
			    				'unit_label' => '',
			    				'type' 	=> 'text',
			    				'value'	=> ''
		    				],
							[
		    					'name'	=> 'tenaga_maksimum',
    ...
    public function _callback_service($value = '', $primary_key = null) {

		$content = $this->Products->find(['id'=>$primary_key]);
		$service = json_decode($content->service, true);
		$return['service'] = $service;
		if (!$return['service']) {
	    	$jsons = ['service' => [
					    	'service_1' 	=> [
				    		'name'	=> 'service_1',
		    				'label' => 'Free Service 1 (1.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
	    				],
				    	'service_2' 	=> [
				    		'name'	=> 'service_2',
		    				'label' => 'Free Service 2 (4.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_3' 	=> [
				    		'name'	=> 'service_3',
		    				'label' => 'Free Service 3 (4.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_4' 	=> [
				    		'name'	=> 'service_4',
		    				'label' => 'Free Service 4 (8.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_5' 	=> [
				    		'name'	=> 'service_5',
		    				'label' => 'Free Service 5 (12.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_6' 	=> [
				    		'name'	=> 'service_6',
		    				'label' => 'Service 6 (16.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_7' 	=> [
				    		'name'	=> 'service_7',
		    				'label' => 'Service 7 (20.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			],
				    	'service_8' 	=> [
				    		'name'	=> 'service_8',
		    				'label' => 'Service 8 (24.000Km)',
		    				'type' 	=> 'textarea',
		    				'value' => ''
		    			]
		    			]
    	...
    	$content = '<div class="tabs">';
	    	...
	    	foreach ($jsons as $key => $json) {
	    		foreach ($json as $val) {
					$content .= '<li><a style="font-size:1.0em" href="#'.$val['name'].'-tab">'.$val['label'].'</a></li>';
				}
	    	}
	    	$content .= '</ul>';
	    	foreach ($jsons as $key => $json) {
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Media Title
            $output->page_title = lang('Motorcycle').' Listings';
            // Set Main Template
...
/* Location: ./application/module/product/controllers/motorcycle.php */
