...
class Contact extends Admin_Controller {
    ...
    public function index() {
	    ...
	    // Set our Grocery CRUD
            ...
            $crud->where('type','head_office');
            $crud->or_where('type','branch_office');            
            // Set CRUD subject
            $crud->set_subject(lang('Contact'));                            
            // Set columns
            $crud->columns('subject','location','phone','fax','zipcode','status','added','modified');			
			// The fields that user will see on add and edit form
			$crud->fields('subject','url','type','location','phone','fax','zipcode','status','added','modified');
            // Changes the default field type
            $crud->field_type('url', 'hidden');
            //$crud->field_type('location', 'textarea');			
			$crud->field_type('added', 'hidden');
			$crud->field_type('modified', 'hidden');
			// Unset texteditor
			$crud->unset_texteditor('location');
			// Set unique fields
			$crud->unique_fields('subject','url');
			
			$crud->field_type('type','dropdown', 
			array(
				'head_office' 	=> lang('Head Office'),
				'branch_office' => lang('Branch Office')
				)
			);
   ...
   public function _callback_translate ($value, $row) {
    ...
    private function load($crud, $nav) {
        ...
        if ($crud->getState() == 'list') {
            // Set Contact Title 
            $output->page_title = lang('Contact');
            // Set Main Template
