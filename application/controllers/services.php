...
class Services extends Public_Controller {
    ...
    public function __construct() {
        ...
        $this->emails_register = $this->Contacts->findAll(['attribute'=>'email_register','status'=>'publish'],'email');
        // Set email survey to sent
        $this->emails_survey = $this->Contacts->findAll(['attribute'=>'email_customer_survey','status'=>'publish'],'email');
        // Set email booking service to sent
        $this->emails_booking = $this->Contacts->findAll(['attribute'=>'email_booking_service','status'=>'publish'],'email');
    ...
    public function index() {
        ...
        $data['page_title'] =  'Official site Suzuki After Sales';
        ...
        // New service date format
        $select_month = [];
        foreach ($periods as $period) {
            $select_month[$period->format('F')] = $period->format('U');
        }
        ...
        // Load JS execution
        //$data['js_inline'] = 'openPop("survey");';

        // Set main template
        $data['main']       = 'services/page/service/index';

        // Set meta description for html tags in template
        $this->site_description = $this->clean_tags('Official site Suzuki After Sales - Service Indonesia | '.$this->site_name);
...
*/
    public function service_automobile_booking()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $saturday   = $attribute->setting_zero->value;
            $sunday     = $attribute->setting_one->value;
            ...
            if (!$saturday && date('l', $post['date']) == 'Saturday') {

                $response['message'] = 'Mohon maaf, di tempat kami '.$dealer->name.' belum bisa melayani booking service di hari Sabtu.';

            } else if (!$sunday && date('l', $post['date']) == 'Sunday') {

                $response['message'] = 'Mohon maaf, di tempat kami '.$dealer->name.' belum bisa melayani booking service di hari Minggu.';

            }
            // Check if dealer is close or not
            else if (!$close) {

                // Select query selecting the booking data
                ...
                if ($select->num_rows() < $limit)
                {
                    ...
                    unset($post['province']);
                    unset($post['select_month']);
                    unset($post['select_hour']);
                    ...
                    if ($insert)
                    {
                        ...
                        $notification   = $this->Notifications->insert([
                            ...
                            'text'      => $booking->name . ' Sudah melakukan Booking Online Service di dealer anda',
                            'status'    => 1,
                            'added'     => time()
                            ]);
...
Untuk itu, kami akan merespon permintaan Booking Service yang Anda kirimkan kepada kami.');
    ...
    public function service_automobile()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Automobile - Official site Suzuki After Sales';
    ...
    public function service_automobile_tips($detail = null)
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Automobile Tips - Official site Suzuki After Sales';
    ...
    public function service_motorcycle()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Motorcycle - Official site Suzuki After Sales';
    ...
    public function service_marine()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Marine - Official site Suzuki After Sales';
    ...
    public function service_layanan24jamsera()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Layanan 24 Jam Suzuki Emergency Roadsite Assistant - Official site Suzuki After Sales';
    ...
    public function service_halosuzuki()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  lang('Service') . ' Halo Suzuki - Official site Suzuki After Sales';
    ...
    public function service_sgo()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] = 'Suzuki Genuine Oil - Official site Suzuki After Sales';
    ...
    public function service_sgp()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  'Suzuki Genuine Parts - Official site Suzuki After Sales';
    ...
    public function service_sgc()
    {
        ...
        // Set site title page with module menu
        $data['page_title'] =  'Suzuki Genuine Chemical - Official site Suzuki After Sales';
    ...
    public function auth_forgot()
    {
        if ($this->input->is_ajax_request() && $this->input->post()) {
            ...
            $select = $this->db->query("SELECT id, email, fullname, username, type, confirmed, approved, status FROM tbl_members WHERE email = '{$email}' AND username != '' LIMIT 0,1");
                ...
                if ((int)$row->confirmed == 1)
                {
                    if ((int)$row->approved == 1)
                    {
                        if ((int)$row->status == 1)
                        {
                            ...
                            if ($this->Members->update($row->id,['password' => sha1($hash)])) {

                                // Set subject
                                $subject = $this->title_name->value.' - Account';

                                // Data to send to public
                                $message['site_name']       = $this->title_name->value;
                                $message['site_link']       = base_url();
                                $message['header']          = 'Account Password';
                                $message['messages']        = '<b>'.$row->email.'</b>, berikut password baru anda : <b>'.$hash.'</b>.';
                                $message['site_copyright']  = $this->copyright->value;
                                ...
                                // Set response
                                $response['success'] = true;
                                $response['message'] = 'Silahkan cek email anda '.$row->email.' untuk melanjutkan!';
...
*/
    public function rest_booking_check_html($dealer_id)
    {
        ...
        $member_id  = $this->DealerNetworks->findBy('id',$dealer_id,'dealer_member_id');
        $config     = $this->Members->findBy('id',$member_id->dealer_member_id,'attribute');
        $return     = json_decode($config->attribute);

        $result['open']     = (int) $return->setting_four->value;
        $result['close']    = (int) $return->setting_five->value;
    ...
    public function product_static($type, $sps_url = null){
        ...
        if ($sps_url == 'periodic-service' || $sps_url == 'periodic-servicing' || $sps_url == 'periodic-maintenance') {
            ...
            $static_pages = $this->Products->findAll(['type'=>$type,'is_service'=>1],'id,subject,type,media_plain,thumbnail',['id'=>'ASC']);
        ...
        // Set site title page with module menu
        $data['page_title'] = $title_breacrumb . ' - Official site Suzuki After Sales';
