...
<!--                                    </div>-->
                        <div class="post-info">
                            <!-- Selengkapnya button -->
                            <a href="<?=base_url('services/automobile/tips/' . $tip->sts_url);?>" title="<?=$tip->sts_subject;?>"><h1 class="grid-title">
                                    <?=$tip->sts_subject;?>
                                </h1></a>
                            <div class="post-meta">
                            <span> <span><a href="<?=base_url('services/automobile/tips/' . $tip->sts_url);?>"><?=date('F d, Y',strtotime($tip->sts_publish_date));?></a></span></span>
                            </div>
                            <div class="post-content">
                                <p class="intro">
                                    <?=character_limiter($tip->sts_text, 500);?>
                                </p>
                                <!-- Selengkapnya button -->
                                <a href="<?=base_url('services/automobile/tips/' . $tip->sts_url);?>" class="icon-button">Selengkapnya<i class="fa fa-chevron-right"></i></a>
                            </div>
                        </div>
                    </li>
                <?php
...
<!--                        </div>-->
        ...
        <aside class="one-third grey">
            <div id="sidebar">
                ...
                <div class="widget">
                    <h5 class="stripe-heading">TIPS MOBIL</h5>
                    <div class="widget-content">
                        <ul class="recent-posts">
                        <?php
                        foreach ($tips as $tip) {
                        ?>
                            <li><a href="<?=base_url('services/automobile/tips/' . $tip->sts_url);?>"><?=$tip->sts_subject;?></a>
                                <span><?=date('F d, Y',strtotime($tip->sts_publish_date));?></span></li>
                        <?php
                        }
                        ?>
                        </ul>
