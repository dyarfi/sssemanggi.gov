            ...
            <ul class="grid alt-posts" id="posts">
                <?php foreach ($rows as $row) { ?>           
                    <li class="post">
                        ...
                        <?php } ?>
                        <div class="post-info">
                                <a href="<?php echo base_url('promo/'.$row->url);?>" title="Read Post"><h1 class="grid-title"><?php echo $row->subject;?></h1></a>
                            <div class="post-meta">
                                <span> <span><a href="<?php echo base_url('promo/'.$row->url);?>"><?php echo date('M d, Y',strtotime($row->publish_date));?></a><?php echo ($row->member->fullname) ? ' Oleh : '.$row->member->fullname:'';?></span></span>
                            </div>
                            <div class="post-content">
                                ...
                                <!-- Selengkapnya button -->
                                <a href="<?php echo base_url('promo/'.$row->url);?>" class="icon-button">Selengkapnya<i class="fa fa-chevron-right"></i></a>
                            </div>
                        </div>                                
                    </li>                   
                <?php } ?>                                                
